...
use Data::Dumper;
my $i = 0;
foreach my $name (sort(listMaps("./patches/extracted"))) {
	$i++;
	print "$i\t$name\n";
	gat2fld("./patches/extracted/$name.gat", "./patches/openkore_fields/$name.fld", readWaterLevel("./patches/extracted/$name.rsw"));
}
...
sub listMaps {
	...
	opendir($handle, $dir);
	my @list = grep { /\.gat$/i && -f "./patches/extracted/$_" } readdir $handle;
	closedir $handle;
